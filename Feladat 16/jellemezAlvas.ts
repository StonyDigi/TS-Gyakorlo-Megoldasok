...
function jellemezAlvas() {
    ...
    const alvasIdoInput = document.getElementById("alvasIdoInput") as HTMLInputElement;
    const alvasIdo = parseInt(alvasIdoInput.value) || 0;

    // Érvényes adatok ellenőrzése
    if (!isNaN(alvasIdo) && alvasIdo >= 0) {
        // Alvási idő jellemezése
        const jellezes = jellemezAlvasLogika(alvasIdo);
...
// Függvény az alvásidejének jellemezéséhez
function jellemezAlvasLogika(alvasIdo: number): string {
